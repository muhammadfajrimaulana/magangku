...
$mahasiswa_id = $_GET['mahasiswa_id']; // Ambil user_id diperlukan
$tanggal = $_GET['tanggal']; // Ambil tanggal, default ke hari ini
...
WHERE pm.user_id = $mahasiswa_id AND pm.tanggal = '$tanggal'
...
if ($result) {
    ...
    if ($result->num_rows > 0) {
        ...
        $sql_masuk = "SELECT tanggal, TIME(waktu) AS waktu_masuk, path_foto AS foto_masuk, lokasi AS lokasi_masuk FROM presensi_masuk WHERE pembimbing_id = $pembimbing_id ORDER BY tanggal DESC, waktu DESC";
        ...
        $sql_pulang = "SELECT tanggal, TIME(waktu) AS waktu_pulang, path_foto AS foto_pulang, lokasi AS lokasi_pulang FROM presensi_pulang WHERE pembimbing_id = $pembimbing_id ORDER BY tanggal DESC, waktu DESC";
